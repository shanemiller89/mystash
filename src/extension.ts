...
import { StashProvider, StashFileDecorationProvider, StashDragAndDropController } from './stashProvider';
...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(authService.onDidChangeAuthentication(() => {
				...
				setTimeout(async () => {
					try {
						...
						if (stashes.length > 0) {
							...
							await treeView.reveal(newItem, { select: true, focus: false, expand: true });
						}
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('mystash.apply', async (item?: StashItem) => {
			...
			if (result.success && result.conflicts) {
				...
				stashProvider.setMessage('$(warning) Last apply had merge conflicts — resolve manually');
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('mystash.search', async () => {
			...
			if (query === undefined) { return; } // Escape
			stashProvider.setSearchQuery(query);
			await vscode.commands.executeCommand('setContext', 'mystash.isSearching', query.length > 0);
		})
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('mystash.applySelected', async (_item?: StashItem, allItems?: StashItem[]) => {
			const items = allItems && allItems.length > 0 ? allItems : treeView.selection.filter((s): s is StashItem => s instanceof StashItem);
			if (items.length === 0) {
			...
			await vscode.window.withProgress(
				{ location: vscode.ProgressLocation.Notification, title: 'Applying stashes…', cancellable: false },
				async (progress) => {
					for (const item of items) {
						progress.report({ message: `${item.stashEntry.name}…` });
						const result = await gitService.applyStash(item.stashEntry.index);
						if (result.success && result.conflicts) { conflictCount++; }
			...
			if (conflictCount > 0) {
				stashProvider.setMessage(`$(warning) Batch apply: ${conflictCount} stash${conflictCount !== 1 ? 'es' : ''} had conflicts`);
			}
			stashProvider.refresh('post-command');
		})
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('workstash.notes.create', async () => {
			...
			const defaultVisibility = vscode.workspace.getConfiguration('workstash.notes')
				.get<string>('defaultVisibility', 'secret');
			const isPublic = defaultVisibility === 'public';

			try {
				await vscode.window.withProgress(
					{ location: vscode.ProgressLocation.Notification, title: 'Creating note…', cancellable: false },
					async () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('workstash.notes.delete', async (item?: GistNoteItem) => {
			...
			try {
				await vscode.window.withProgress(
					{ location: vscode.ProgressLocation.Notification, title: 'Deleting note…', cancellable: false },
					async () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('workstash.notes.toggleVisibility', async (item?: GistNoteItem) => {
			if (!item) { return; }
			...
			const confirm = await vscode.window.showWarningMessage(
				...
				{ modal: true },
				`Make ${targetVis}`, 'Cancel'
			);
			if (!confirm || confirm === 'Cancel') { return; }

			try {
				await vscode.window.withProgress(
					{ location: vscode.ProgressLocation.Notification, title: `Toggling visibility…`, cancellable: false },
					async () => {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('workstash.notes.search', async () => {
			...
			await vscode.commands.executeCommand('setContext', 'workstash.notes.isSearching', query.length > 0);
		})
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('mystash.dropSelected', async (_item?: StashItem, allItems?: StashItem[]) => {
			const items = allItems && allItems.length > 0 ? allItems : treeView.selection.filter((s): s is StashItem => s instanceof StashItem);
			if (items.length === 0) {
				vscode.window.showInformationMessage('No stashes selected');
				return;
			...
			await vscode.window.withProgress(
				{ location: vscode.ProgressLocation.Notification, title: 'Dropping stashes…', cancellable: false },
				async (progress) => {
			...
			const parts: string[] = [];
			if (successCount > 0) { parts.push(`${successCount} dropped`); }
			if (failCount > 0) { parts.push(`${failCount} failed`); }
