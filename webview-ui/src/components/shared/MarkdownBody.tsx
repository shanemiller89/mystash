...
/**
 * A render plugin that transforms the final HTML after markdown-it renders.
 * Used to inject feature-specific post-processing (e.g. Mattermost custom emoji)
 * without coupling MarkdownBody to any single feature's store.
 */
export interface MarkdownRenderPlugin {
    /** Unique key for React dependency tracking */
    readonly key: string;
    /** Transform the rendered HTML string. Called in plugin array order. */
...
/**
 * Replace :shortcode: patterns in text with emoji Unicode chars.
 * Custom emoji (e.g. Mattermost) are handled via renderPlugins.
 * Runs AFTER markdown-it renders so we operate on the final HTML text nodes.
 */
function renderEmojiInHtml(html: string): string {
...
interface MarkdownBodyProps {
    ...
    currentUsername?: string | null;
    /** Optional render plugins that post-process the HTML after markdown-it renders */
    renderPlugins?: MarkdownRenderPlugin[];
}
 ...
 */
export const MarkdownBody: React.FC<MarkdownBodyProps> = ({ content, className = '', currentUsername, renderPlugins }) => {
    // Stable reference for plugin keys (used in useMemo dependency)
    const pluginKeys = renderPlugins?.map((p) => p.key).join(',') ?? '';

    const html = useMemo(() => {
        let result = sanitizeHtml(md.render(content));

        // Built-in: Unicode emoji shortcodes (no store dependency)
        result = renderEmojiInHtml(result);

        // Run any injected plugins (e.g. Mattermost custom emoji)
        if (renderPlugins) {
            for (const plugin of renderPlugins) {
                result = plugin.transformHtml(result);
            }
        }

        result = highlightMentions(result, currentUsername ?? null);
        result = neutralizeGitHubLinks(result);

        return result;
    }, [content, currentUsername, pluginKeys, renderPlugins]);
